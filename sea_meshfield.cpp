							...
							int nNumBlockX, int nNumBlockZ, float fBlockSizeX, float fBlockSizeZ)
{
	...
	g_Center = XMFLOAT3(0.0f, 0.0f, 2000.0f);		// 波の発生場所
	g_Time = 0.0f;								// 波の経過時間(＋とーとで内側外側になる)
	g_wave_frequency = 1.5f;					// 波の周波数（上下運動の速さ）
	g_wave_correction = 0.01f;					// 波の距離補正（変えなくても良いと思う）
	g_wave_amplitude = 4.5f;					// 波の振幅(波の高さ)
...
void UpdateSeaMeshField(void)
{
	...
	for (int z = 0; z < g_nNumBlockZField; z++)
	{
		for (int x = 0; x < g_nNumBlockXField; x++)
		{
			...
			float dx = g_Vertex[z * (g_nNumBlockXField + 1) + x].Position.x;
			float dz = g_Vertex[z * (g_nNumBlockXField + 1) + x].Position.z - g_Center.z;
